...
from ..config.constants import LLM_DEFAULT_MODEL, LLM_REQUEST_TIMEOUT_SECONDS, OPENAI_DEFAULT_API_URL
...
class ChatLLMClient:
    ...
    def _post_chat(self, payload: dict[str, Any]) -> ChatLLMResponse | None:
        ...
        if not content:
            ...
            self.last_error = f"OpenAI API error: {self._extract_error(error)}" if error else "empty response content"
            self.last_response_preview = ""
            return None
