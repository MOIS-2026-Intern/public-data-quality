...
def unique_artifact_path(directory: Path, filename: str) -> Path:
    ...
    token = uuid4().hex
    return directory / f"{stem}{_ARTIFACT_TOKEN_DELIMITER}{token}{suffix}"
...
def public_download_name(filename: str, *, default_suffix: str = ".xlsx") -> str:
    ...
    if delimiter and len(token) == 32 and all(character in "0123456789abcdef" for character in token):
