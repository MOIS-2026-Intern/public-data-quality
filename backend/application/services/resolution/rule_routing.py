...
from backend.config.column_rules import (
    ROUTING_NON_UNIQUE_NAME_EXCLUDED_RULES,
...
def looks_non_unique_name_column(column: ColumnProfile) -> bool:
    name = f"{column.raw_name} {column.normalized_name}"
    return any(token in name for token in ROUTING_NON_UNIQUE_NAME_TOKENS)
...
def apply_llm_route(column: ColumnProfile, payload: dict[str, Any]) -> ColumnProfile:
    ...
    if is_non_unique_name:
        assigned_rules = [
            ...
            if rule_id not in ROUTING_NON_UNIQUE_NAME_EXCLUDED_RULES
        ]
